...
<div id="emp-container">
		...
		<form name="empSearchFrm" action="">
			...
			<table id="tbl-search">
				<tr>
					<th colspan="2">
						<select name="searchType">
							...
							<option value="emp_id" ${param.searchType== "emp_id" ? 'selected="selected"' : '' }>사번</option>
							<option value="emp_name" ${param.searchType== "emp_name" ? 'selected="selected"' : '' }>사원명</option>
							<option value="email" ${param.searchType== "email" ? 'selected="selected"' : '' }>이메일</option>
							<option value="phone" ${param.searchType== "phone" ? 'selected="selected"' : '' }>전화번호</option>
						</select>
						&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<tr>
					<th>성별</th>
					<td>
						<input type="radio" name="gender" value='남' id="gender0" ${param.gender== "남" ? 'checked="checked"' : '' }/>
						<label for="gender0">남</label>
						<input type="radio" name="gender" value='여' id="gender1" ${param.gender== "여" ? 'checked="checked"' : '' }/>
	...
	<script>
	function reset(){
		$("input[name='searchKeyword']").children("option").attr("selected", "false");
	};
	</script>
